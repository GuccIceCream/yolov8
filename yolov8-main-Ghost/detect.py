...
def parse_opt():
    ...
    parser.add_argument('--weight', type=str, default='D:\\重做消融实验\\yolov8m-base\\weights\\best.pt', help='training model path')
    parser.add_argument('--source', type=str, default='D:\\alldata\\VisDrone2019\\VisDrone2019-DET-val\\images', help='source directory for images or videos')
    parser.add_argument('--conf', type=float, default=0.4, help='object confidence threshold for detection')
...
if __name__ == '__main__':
    ...
    if opt.mode == 'predict':
        model.predict(source=opt.source, **transformer_opt(opt),save_txt=True)
    elif opt.mode == 'track':
